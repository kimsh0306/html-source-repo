...
<%@ include file="./JEasyUICommon.jsp" %>
	<script type="text/javascript">
		//여기가 전변자리이다.
		//전역변수는 그 페이지 내에서 계속 유지되므로 업무 프로세스가 새로 시작할 땐 처음 값으로 반드시 초기화
		var g_address=''; //사용자가 선택한 주소정보 담기
		var g_cnt = 0;//수정 시 한 건만 선택되었는지 체크함.
		var g_empno = 0;
		...
		function empUPD(){
			if(g_cnt>1){
				$.messager.alert('Info','한 번에 한 건만 수정할 수 있습니다.');
				empnoList();
				return;//empUPD함수 탈출
			}
			if(g_empno == 0){
	...
	<script type="text/javascript">
		function empList(){
			//조회될 때마다 새로 업무가 시작되도록 전역변수 초기화 해 줄 것.
			g_cnt = 0;
			$("#dg_emp").datagrid({
				 url:"jsonEmpList.jsp"
...
//				,singleSelect:true
	...
	<div id="dlg_upd" data-options="closed:true, title:'사원정보 등록', footer:'#d_upd', modal:'true'" class="easyui-dialog" style="width:100%;max-width:580px;padding:30px 60px">
		<form id="f_upd">
			<div style="margin-bottom:10px">
			<input class="easyui-textbox" id="empno" name="u_empno" label="사원번호" labelPosition="top" data-options="prompt:'Enter a EmpNO'" style="width:210px">
			</div>
			<div style="margin-bottom:10px">
			<input class="easyui-textbox" id="ename" name="u_ename" label="사원명" labelPosition="top" data-options="prompt:'Enter a ENAME'" style="width:210px">
			</div>
			<div style="margin-bottom:10px">
			<input class="easyui-textbox" id="mem_job" name="u_mem_job" label="JOB" labelPosition="top" data-options="prompt:'Enter a JOB'" style="width:210px">
			</div>
			<div style="margin-bottom:10px">
			<input class="easyui-textbox" id="mem_hiredate" name="u_mem_hiredate" label="입사일자" labelPosition="top" data-options="prompt:'Enter a HIREDATE'" style="width:210px">
			</div>
			<div style="margin-bottom:10px">
			<input class="easyui-textbox" id="mem_sel" name="u_mem_sel" label="급여" labelPosition="top" data-options="prompt:'Enter a SEL'" style="width:210px">
			</div>
			<div style="margin-bottom:10px">
			<input class="easyui-numberbox" id="comm" name="u_comm" label="인센티브" labelPosition="top" data-options="prompt:'Enter a 인센티브'" style="width:210px">
			</div>	
			<div style="margin-bottom:10px">
			<input class="easyui-combobox" id="mem_deptno" name="u_mem_deptno" label="부서번호" labelPosition="top" style="width:210px" 
					data-options="prompt:'Enter a 부서번호'
						         ,valueField: 'DEPTNO' 	<!-- 실제로 넘어가는 값 -->
						         ,textField: 'DNAME' 	<!-- 화면에 보여지는 부분 -->
						         ,url: './jsonDeptList.jsp'
